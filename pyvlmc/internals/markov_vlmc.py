...
from pyvlmc.internals.vlmc import all_contexts, copy_tree, fit_vlmc

def memoryless_vlmc(train_data, marginal=True, *args, **kwargs):
    return make_fsm_closure(expand(fit_vlmc(train_data, *args, **kwargs)))
...
def make_fsm_closure(mc):
    ...
    for c in contexts:
        map(add_context, [c[0:i] for i in range(len(c))])
    return cp_mc
