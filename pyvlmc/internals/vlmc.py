...
import numpy as np
import random
from pyvlmc.internals.contexttree import ContextTreeNode
import bisect
import heapq as hq
...
def fit_context_tree(train_data, thresh, depth=1,
    ...
    if filter_set is None:
        filter_set = range(1, len(train_data))
    if alphabet is None:
        alphabet = set(train_data)
...
def expand_nd(mc):
    ...
    for label, child in iter(mc.children.items()):
...
def all_contexts(mc, path=None, implicit=False, interior=False):
...
def predict_next(mc, context, pred_type="prob"):
    ...
    maxkey = max(iter(child.prob_dist.items()), key=itemgetter(1))
    return maxkey[0]
...
def state_marginal_probs(mc, interior=False):
    ...
    for k, v in iter(context_mp.items()):
        if len(k) == 0:
            print("Bad context?")
            print(k)
            continue
...
def simulate(mc, sim_len, discard=None):
    ...
    if discard is None or not isinstance(discard, (int, float)):
        discard = order_mc * n_context_discard
    sim = random.sample(mc.alphabet, 1) * (discard + sim_len)
    for i in range(order_mc, discard + sim_len):
...
def simulate_holding_time(mc, sym, nsim=1):
    ...
    return [q_np_head_geom_tail(head_c, tail_p, random.random()) for _ in range(nsim)]
...
def copy_tree(mc):
    newcounts = {k: v for k, v in iter(mc.counts.items())}
    newprobs = {k: v for k, v in iter(mc.prob_dist.items())}
    if not mc.children:
        return ContextTreeNode(newcounts, {}, newprobs)
    return ContextTreeNode(newcounts, {k: copy_tree(v) for k, v in iter(mc.children.items())}, newprobs)
...
def format_node(node, nodelabel, fmt, rnd=3):
    if fmt != 'prob':
        kidcounts = ["%s: %d" % (str(k), v) for k, v in iter(node.counts.items())]
        return str(nodelabel) + "(%s | %d)" % (", ".join(kidcounts), node.totalcount)
    fmtstr = "%%s: %%.%df" % (rnd)
    return str(nodelabel) + "(%s)" % (
        ", ".join([fmtstr % (str(k), round(v, rnd)) for k, v in iter(node.prob_dist.items())]))
