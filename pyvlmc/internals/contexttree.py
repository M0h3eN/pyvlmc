...
def ContextTreeNode(counts, children, prob_dist=None):
    totalcount = sum(iter(counts.values()))
    if prob_dist is None:
        if totalcount != 0:
            prob_dist = {s: c/totalcount for s, c in iter(counts.items())}
        else:
            prob_dist = {s: 0 for s in iter(counts.keys())}
...
def update_next_symbol_probs(self, posterior):
    if posterior.keys() != self.prob_dist.keys():
        ...
        for k in iter(self.prob_dist.keys()):
...
def to_dict(self):
    ...
    if not self.children:
        root['children'] = {}
        return root
    root['children'] = {k: v.to_dict() for k, v in iter(self.children.items())}
    return root
...
def from_dict(c_dict):
    ...
    children = {k: from_dict(v) for k, v in iter(c_dict['children'].items())}
