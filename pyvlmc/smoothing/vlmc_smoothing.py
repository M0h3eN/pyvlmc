...
from pyvlmc.internals import vlmc as vl
...
def forward(f_hat, a_hat, p_hat, p_hat_emb, real_states, pi_hat=None):
    ...
    for t in range(1, f_over_p.shape[0]):
...
def embed_vlmc(mc, interior=True):
    ...
    emb_matrix.ix_to_state.update(dict(enumerate(iter(emb_dict.keys()))))
    emb_matrix.state_to_ix.update({v: k for k, v in iter(emb_matrix.ix_to_state.items())})
    for k, t in iter(emb_dict.items()):
        for l, v in iter(t.items()):
